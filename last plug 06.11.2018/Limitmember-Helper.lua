...
if msg.query and msg.query:sub(1,6) == "Addl:-" and msg.query:gsub("Addl:-",""):match('%d+') and is_sudo(msg) then
	...
	send_inline(msg.id,'settings','Group Option','Tap Here','` ●•۰ به بخش تنظیمات اد اجباری خوشآمدید لطفا پس اعمالِ یک تغییر 2 ثانیه به ربات جهت انجام فرایند مهلت دهید و سپس دوباره تغییر خود را انجام دهید..`','Markdown',keyboard)
end
--######################################################################--
...
if mr_roo[1] == '/exitadd' then
   ...
   elseif not is_req(mr_roo[2], msg.from.id) then 
        Canswer(msg.cb_id, "شما این فهرست را درخواست نکردید" ,true) 
		 ...
		 text = 'تنظیمات محدودیت کاربر بسته شده \n ➖🌹➖AxE➖🌹➖'
